...
export class ProjectEstimateComponent implements OnInit, AfterViewInit {
  ...
  aiPoints(): string[] {
    const estimate = (this.response()?.result['estimate'] as Record<string, unknown>) || {};
    return [
      `Remaining estimate is ${this.getValue('remaining_estimate_usd')}, which is the primary control number.`,
      `The largest overrun pressure should be validated across procurement, market, and project tracker drivers.`,
      `Current estimate risk is ${String(estimate['risk_level'] ?? 'unknown')}, so remediation priority should be set before the next forecast cycle.`
    ];
  }

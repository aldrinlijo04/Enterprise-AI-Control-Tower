...
export function ModulePanel({ title, iconKey, accent = "#06d6a0", badge, children, noPad }) {
  return (
    <div className="module-panel" style={{ "--accent": accent }}>
      <div className="module-header">
        {iconKey && (
          <span className="module-icon" style={{ color: accent }}>
            <Icon d={ICON_PATHS[iconKey]} size={15} />
          </span>
        )}
        <span className="module-title">{title}</span>
        {badge !== undefined && (
          <span className="module-badge" style={{ background: accent }}>{badge}</span>
        )}
      </div>
      <div className={noPad ? "module-body-nopad" : "module-body"}>
        {children}
      </div>
    </div>
  );
